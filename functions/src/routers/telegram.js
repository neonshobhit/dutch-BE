...
bot.onText(/\/addTransaction/, async (msg) => {
	...
	await controller.storeMessage(
		sentMessage.text,
		sentMessage.message_id,
		sentMessage.chat.id,
		Telegram.getString(Telegram.ADD_TRANSACTION),
		Telegram.getString(Telegram.SELECT_EVENT),
	);
});
...
bot.onText(/\/getEvents/, async (msg) => {
	...
	await controller.storeMessage(
		...
		sentMessage.message_id,
		sentMessage.chat.id,
		Telegram.getString(Telegram.LIST_EVENTS),
		Telegram.getString(Telegram.SUCCESS),
	);
});

bot.onText(/\/subscribeEvent/, async (msg) => {
	const events = await controller.listEvents(msg.from);
	if (!events.status) return bot.sendMessage(msg.chat.id, events.error);

	const opt = Telegram.generateMessage(Telegram.SUBSCRIBE_EVENT, events);
	const sentMessage = await bot.sendMessage(msg.chat.id, opt[0], opt[1]);
	...
	await controller.storeMessage(
		...
		sentMessage.message_id,
		sentMessage.chat.id,
		Telegram.getString(Telegram.SUBSCRIBE_EVENT),
		Telegram.getString(Telegram.SELECT_NOTIFY_EVENT),
	);
});
bot.onText(/\/unsubscribeEvent/, async (msg) => {
	const events = await controller.listEvents(msg.from);
	if (!events.status) return bot.sendMessage(msg.chat.id, events.error);

	const opt = Telegram.generateMessage(Telegram.UNSUBSCRIBE_EVENT, events);
	const sentMessage = await bot.sendMessage(msg.chat.id, opt[0], opt[1]);
	...
	await controller.storeMessage(
		...
		sentMessage.chat.id,
		Telegram.getString(Telegram.SUBSCRIBE_EVENT),
		Telegram.getString(Telegram.DESELECT_NOTIFY_EVENT),
	);
});
...
bot.onText(/\/linkAccount/, async (msg) => {
	...
	await controller.storeMessage(
		...
		response.message_id,
		msg.chat.id,
		Telegram.getString(Telegram.LINK_ACCOUNT),
		Telegram.getString(Telegram.LINKING_EMAIL_ID),
	);
});
...
bot.onText(/\/getProfile/, async (msg) => {
	...
	await controller.storeMessage(
		...
		sentMessage.message_id,
		sentMessage.chat.id,
		Telegram.getString(Telegram.GET_PROFILE),
		Telegram.getString(Telegram.SUCCESS),
	);
});
...
bot.on("text", async (msg) => {
	...
	if (!isReply) {
		...
		if (!notification.status)
			return bot.sendMessage(msg.chat.id, notification.error);
	...
	if (lastMsg.msg.nextFlow === Telegram.LINKING_EMAIL_ID) {
		const response = await controller.linkAccount(
			msg.from,
			msg.text.trim(),
		);
		if (!response.status)
			return bot.sendMessage(msg.chat.id, response.error);
	...
	} else if (lastMsg.msg.nextFlow === Telegram.SELECT_NOTIFY_EVENT) {
		msg.text = msg.text.replace("@bill_Splitbot", "").trim();
		metadata.eventId = msg.text;

		const subscriptionStatus = await controller.subscribeEvent(
			msg,
			metadata,
		);
		if (!subscriptionStatus.status)
			return bot.sendMessage(msg.chat.id, subscriptionStatus.error);
	...
	} else if (lastMsg.msg.nextFlow === Telegram.DESELECT_NOTIFY_EVENT) {
		msg.text = msg.text.replace("@bill_Splitbot", "").trim();
		metadata.eventId = msg.text;

		const unsubscriptionStatus = await controller.unSubscribedEvents(
			msg,
			metadata,
		);
		if (!unsubscriptionStatus.status)
			return bot.sendMessage(msg.chat.id, unsubscriptionStatus.error);
		...
		if (isNaN(amount))
			return bot.sendMessage(msg.chat.id, "Please send proper amount.");
		...
		if (!members.status)
			return bot.sendMessage(msg.chat.id, "Empty members list.");
